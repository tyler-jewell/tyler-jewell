...
assert '[[ "$(classify_mesh_role yes no no)" == "client-only" ]]' "bin no gpu no ep → client-only"
assert '[[ "$(classify_mesh_role no no no)" == "unavailable" ]]' "no bin no endpoint → unavailable"
assert '[[ "$(classify_mesh_role no yes no)" == "client-peer" ]]' "endpoint without local bin → client-peer (not server)"
assert '[[ "$(classify_mesh_role no yes yes)" == "client-peer" ]]' "endpoint + gpu but no bin → client-peer"
assert '[[ "$(classify_mesh_role yes yes no)" == "client-peer" ]]' "bin+ep without gpu → client-peer (cannot serve)"

# can_serve is binary+GPU only — never implied by endpoint alone
assert 'mesh_can_serve yes yes' "bin+gpu → can_serve"
assert '! mesh_can_serve no yes' "no bin + endpoint (gpu yes) → cannot serve"
assert '! mesh_can_serve no no' "no bin no gpu → cannot serve"
assert '! mesh_can_serve yes no' "bin without gpu → cannot serve"
assert '! mesh_can_serve no yes' "remote peer ep only → cannot serve"
# Role back-compat: client-peer must not serve
assert '! mesh_can_serve client-peer' "role client-peer → cannot serve"
assert 'mesh_can_serve server' "role server → can serve"
assert 'mesh_can_serve server-capable' "role server-capable → can serve"
assert '! mesh_can_serve client-only' "role client-only → cannot serve"

assert 'mesh_can_consume yes no' "endpoint up → can_consume"
assert 'mesh_can_consume no yes' "binary only → can_consume (client tooling)"
assert '! mesh_can_consume no no' "no ep no bin → cannot consume"
assert 'mesh_can_consume server' "role server can consume"
assert 'mesh_can_consume client-peer' "role client-peer can consume"
assert 'mesh_can_consume client-only' "role client-only can consume"
